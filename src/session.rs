...
fn load_session(path: &std::path::Path) -> Option<Session> {
    ...
    let (user_messages, task_summaries) = load_events(path);
...
fn load_checkpoints(session_path: &std::path::Path) -> Vec<Checkpoint> {
    ...
    content
        .lines()
        .filter(|line| line.starts_with("| ") && !line.contains("---") && !line.contains("Title"))
        .filter_map(|line| {
